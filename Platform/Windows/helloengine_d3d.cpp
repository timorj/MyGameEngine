...
ID3D11Device *g_pDev = nullptr; //pointer to our Device interface
ID3D11DeviceContext *g_pDevcon= nullptr; //pointer to our Device Context

ID3D11RenderTargetView *g_pRTView = nullptr;

ID3D11InputLayout *g_pLayout = nullptr; //pointer to the inputLayout
ID3D11VertexShader *g_pVS = nullptr; //pointer to the vertex shader
ID3D11PixelShader *g_pPS = nullptr; //pointer to the pixel shader
...
void InitGraphics(){
    ...
    bd.Usage = D3D11_USAGE_DYNAMIC; //write the access by cpu and gpu
    bd.ByteWidth = sizeof(VERTEX) * 3; //three vertex with a triangle
...
HRESULT CreateGraphicsResources(HWND hWnd){
    ...
    if (g_pSwapchain == nullptr)
    {
        ...
        scd.BufferDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM; //use 32_bit color
        ...
        if (hr == E_INVALIDARG)
        {
            D3D11CreateDeviceAndSwapChain(NULL, D3D_DRIVER_TYPE_HARDWARE,
                NULL, 0, &FeatureLevelSupported, 1, 
                D3D11_SDK_VERSION,
                &scd,
                &g_pSwapchain,
                &g_pDev,
